...
**⚠️ 重要提示**: v2.0.0 是一个重大版本更新，采用全新API，与v1.x版本不兼容。升级后旧播放列表将被清空。
...
- **跨平台支持**: [pathlib](https://docs.python.org/3/library/pathlib.html) (现代路径处理)
...
#### 🚨 破坏性变更
- **API迁移**: 从旧API迁移到新的vkeys.cn API
- **数据格式变更**: 歌曲标识从索引`n`改为唯一ID
- **播放列表不兼容**: 旧版本播放列表数据无法直接使用，需重新添加歌曲
- **自动数据迁移**: 程序会自动备份旧数据到`playlists_backup_v1.json`并清空播放列表

#### 🎉 新功能
- **增强的歌词支持**:
  - 支持逐字歌词（yrc）用于未来的逐字高亮动画
  - 支持翻译歌词（trans）
  - 支持罗马音/音译（roma）
- **专辑信息**: 搜索结果现在包含专辑信息
- **更稳定的ID系统**: 使用固定唯一ID代替动态索引，提升播放列表稳定性

#### ⚡ 性能优化
- **歌词请求优化**: 歌词单独请求，减少不必要的数据传输
- **更快的歌曲匹配**: 基于ID的精确匹配，无需索引查找

### v1.2.0 - 稳定性和性能重大改进

#### 🔧 核心修复
- **音频设备热切换支持**: 修复Windows系统下声卡切换时音频输出不同步的问题，使用定时器检查机制自动切换到新的默认音频设备
- **线程安全增强**: 统一线程管理机制，解决长时间运行后的概率性闪退问题，确保所有后台线程正确注册和清理
- **优雅关闭处理**: 改进程序关闭流程，确保所有活跃线程、定时器和动画在退出前正确停止，防止意外崩溃
- **播放器错误处理**: 增加全面的QMediaPlayer错误处理机制，提供用户友好的错误提示并自动尝试播放下一首歌曲

#### ⚡ 性能优化
- **歌词显示优化**: 
  - 定时器频率从100ms优化到250ms，减少CPU占用
  - 实现HTML缓存机制，只在必要时重新生成歌词页面
  - 使用二分查找算法快速定位当前歌词行，提升大型歌词文件的处理效率
- **网络性能提升**:
  - 引入全局requests.Session，支持连接池复用和Keep-Alive
  - 实现智能重试策略（3次重试，指数退避）
  - 优化超时设置：连接5s，读取15s，下载30s

#### 🛡️ 稳定性改进
- **临时文件管理**: 统一的临时文件清理机制，防止磁盘空间泄漏和文件句柄累积
- **内存泄漏修复**: 确保线程对象通过`deleteLater()`正确释放，避免长时间运行后的内存问题
- **信号连接优化**: 修复音量动画信号重复连接导致的RuntimeWarning
- **数据存储权限修复**: 
  - 将播放列表数据从程序安装目录迁移到用户AppData目录（`%APPDATA%\MusicDownloader\playlists.json`）
  - 解决安装到Program Files后需要管理员权限才能保存播放列表的问题
  - 实现自动数据迁移，确保现有用户的播放列表无缝转移

#### 🌐 跨平台兼容性
- **现代路径处理**: 全面迁移到`pathlib.Path`，提升Windows/Linux/macOS的路径处理一致性
- **ARM64 Windows支持**: Inno Setup安装包支持ARM64架构的Windows设备（如Surface Pro X）

### v1.1.0 - 界面和功能增强
- 自定义QSS主题系统，基于Catppuccin配色
- 平滑音量调节和淡入淡出效果
- 同步歌词显示功能
- 批量下载和播放列表管理
